...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        self.config.prompt_token_num = model_args.prompt_token_num
        self.config.with_ape = model_args.with_ape
        self.config.with_local = model_args.with_local
        self.config.with_global = model_args.with_global
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_points(self, points):
        ...
        point_features = self.get_model().mm_projector(pos_features.to(torch.float32), local_features, global_features)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(points) is list:
            concat_points = torch.cat([point for point in points], dim=0)
            point_features = self.encode_points(concat_points)
            split_sizes = [point.shape[0] for point in points]
            point_features = torch.split(point_features, split_sizes, dim=0)
            point_features = [x.flatten(0, 1) for x in point_features]
        else:
            point_features = self.encode_points(points)
            point_features.to(points.dtype)
        new_input_embeds = []
            ...
            while point_token_indices.numel() > 0:
                ...
                if getattr(self.config, 'tune_mm_mlp_adapter', False) and getattr(self.config, 'mm_use_pt_start_end',
                    ...
                    cur_new_input_embeds.append(
                        self.get_model().embed_tokens(cur_input_ids[:point_token_start - 1]).detach())
                    cur_new_input_embeds.append(
                        self.get_model().embed_tokens(cur_input_ids[point_token_start - 1:point_token_start]))
                    cur_new_input_embeds.append(cur_point_features)
                    cur_new_input_embeds.append(
                        self.get_model().embed_tokens(cur_input_ids[point_token_start + 1:point_token_start + 2]))
                    if labels is not None:
                        cur_new_labels.append(cur_labels[:point_token_start])
                        cur_new_labels.append(
                            torch.full((cur_point_features.shape[0],), IGNORE_INDEX, device=labels.device,
                                       dtype=labels.dtype))
                        cur_new_labels.append(cur_labels[point_token_start:point_token_start + 1])
                        cur_labels = cur_labels[point_token_start + 2:]
                else:
                    cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids[:point_token_start]))
                    cur_new_input_embeds.append(cur_point_features)
                    if labels is not None:
                        cur_new_labels.append(cur_labels[:point_token_start])
                        cur_new_labels.append(
                ...
                if getattr(self.config, 'tune_mm_mlp_adapter', False) and getattr(self.config, 'mm_use_pt_start_end',
                    ...
                    cur_input_ids = cur_input_ids[point_token_start + 1:]
                point_token_indices = torch.where(cur_input_ids == POINT_TOKEN_INDEX)[0]
    ...
    def initialize_vision_tokenizer(self, model_args, tokenizer):
        if model_args.mm_use_pt_patch_token:
            tokenizer.add_tokens([DEFAULT_POINT_PATCH_TOKEN], special_tokens=True)
            self.resize_token_embeddings(len(tokenizer))

        if model_args.mm_use_pt_start_end:
            num_new_tokens = tokenizer.add_tokens([DEFAULT_PT_START_TOKEN, DEFAULT_PT_END_TOKEN], special_tokens=True)
            self.resize_token_embeddings(len(tokenizer))
        ...
        elif model_args.mm_use_pt_patch_token:
            if model_args.tune_mm_mlp_adapter:
                for p in self.get_input_embeddings().parameters():
                    p.requires_grad = False
                for p in self.get_output_embeddings().parameters():
